...
import (
	...
	"math/rand"
	"net/http"
	"ticket-to-ride/pkg/data"
	"ticket-to-ride/pkg/game"
	"ticket-to-ride/pkg/player"
...
func PlayMoveHandler(w http.ResponseWriter, r *http.Request) {
	...
	if req.Index < 0 || req.Index >= len(edges) {
		http.Error(w, "Invalid route index", http.StatusBadRequest)
		return
	}
	line := (*game.TrainLine)(edges[req.Index])
	if line.P.(*game.TrainLineProperty).Occupied {
		http.Error(w, "Line already occupied", http.StatusBadRequest)
		return
	}
